...
$cols = db::select("tc.CONSTRAINT_TYPE", "c.COLUMN_NAME", "c.DATA_TYPE", "c.IS_NULLABLE", "c.CHARACTER_MAXIMUM_LENGTH")
		->from(
			...
			->leftjoin("INFORMATION_SCHEMA.KEY_COLUMN_USAGE cu")->on("c.TABLE_NAME = cu.TABLE_NAME and c.COLUMN_NAME = cu.COLUMN_NAME")
		...
		->where("c.TABLE_SCHEMA = ".db::str(db::$schema_name))
...
$fks = db::select("fk.TABLE_NAME", "cu.COLUMN_NAME")
		...
		->where("pk.TABLE_SCHEMA = ".db::str(db::$schema_name))
		->where("fk.TABLE_SCHEMA = ".db::str(db::$schema_name))
